...
- Key env vars (non-secret): `ALMS_LLM_PROVIDER` (`openai`|`anthropic`), `ALMS_LLM_MOCK=1`, `DEFAULT_MODEL`, `LLM_BASE_URL`, `ALMS_AGENT_ID`, `ALMS_AUTH_TOKEN`, `ALMS_SANDBOX_ROOT`, `ALMS_SHELL_POLICY`, `ALMS_DATA_DIR` (default: `./data`), `ALMS_WORKSPACE_DIR` (default: `{data_dir}/workspace`), `ALMS_MASTER_KEY` (encrypts `data/secrets.json` at rest with AES-256-GCM), `ALMS_LOG_DIR` (default: `{data_dir}/logs`), `ALMS_LOG_FILE_LEVEL` (default: `debug`), `ALMS_LOG_ROTATION` (`daily`|`hourly`|`never`), `ALMS_LOG_FILE_ENABLED` (`true`|`false`, default: `true`)
