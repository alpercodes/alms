...
- **Pure hierarchy multi-agent** — any agent can spawn subagents via `invoke_agent` tool; results flow up from children to parents; no peer-to-peer messaging between agents. Named subagents (`name` param) get persistent sessions via UUID v5 deterministic identity — conversation history preserved across invocations. Peer mesh (Option 2) is a possible future direction.
