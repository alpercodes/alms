...
**Not yet real**: recursive subagent spawning (subagents can't yet spawn sub-subagents), progress reporting, invoke_agent result truncation (full responses still stored in parent context), `dm_conversation_ended` SSE emission for depth-exceeded path (currently only emitted for `ignore_message` path). See `docs/autonomous-subagents-design.md`.
