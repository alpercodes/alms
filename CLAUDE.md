...
- **Pure hierarchy multi-agent** — any agent can spawn subagents via `invoke_agent` tool; results flow up from children to parents; no peer-to-peer messaging between agents. Named subagents (`name` param) must be pre-registered via `alms agent create`; config (system_prompt, model, posture) loaded from agent registry, workspace attached at `{workspace_dir}/{name}/`. Persistent sessions via UUID v5 deterministic identity — conversation history preserved across invocations. `read_subagent_session` tool for on-demand context retrieval from subagent sessions. Peer mesh (Option 2) is a possible future direction.
