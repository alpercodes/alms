...
- `run_trigger_loop` emits the `dm_conversation_ended` SSE on the DM session with `reason: depth_exceeded`, forwards to each agent's web-chat via `notify_dm_ended_to_webchat`, and enqueues two notification runs.
