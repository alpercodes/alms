...
The peer receives a one-shot notification run — **unless the end was *interrupted***, in which case it gets no run at all: the persisted marker plus its live banner are the operator's delivery, and a `persist_error_marker` record on the session the run would have used is the agent's (#1258 / #1300; see `dm-run-lifecycle.md` hazard 8 for the authoritative description of which reasons are interrupted, why the cut is not the transcript, and how the record reaches the model). Everything in the rest of this section describes the run that a *completed* end still gets. The raw `RunTrigger.input` from `end_conversation()` is a simple marker, but `run_trigger_loop` in `runs.rs` enriches it via `format_dm_ended_notification()`, which produces a richer message including the reason and a `read_messages` hint. For example, when the reason is `Ignored`:
...
The memory bullet used to read *corruption*, and meant it literally: `AgentWorkspace::append_file` was an unlocked read-modify-write, so one of two overlapping appends was silently dropped. That is fixed (#1280, #1294) for **every writer that goes through `AgentWorkspace`** — `append_file`, `workspace_write` with `mode: "write"`, and `PUT /agents/{id}/workspace/{file}`: each runs under the file's exclusive advisory lock, appends land through an append-mode handle, and replacements land through a rename. So no two of *those* writes can land inside one another, and no reader ever observes a partial file, without any caller-side serialisation. The scope is the load-bearing part: `fs_write` reaches the same bytes without going through `AgentWorkspace` at all — an agent's metadata directory sits inside the project-root sandbox, so `fs_write` on `.alms/agents/<name>/memories.md` replaces the file with no sidecar lock, no staging and no per-file default — and the paragraph below is about a loss no lock addresses for any of them.

What no lock covers is a caller that read the file, thought, and then replaced it. The agent's "read" of its memories is the **prompt build**, so the snapshot a `workspace_write` replaces is at best one tool batch old and at worst as old as the run. (This paragraph used to say the prompt was assembled once per run and "carried through every tool iteration". It is not: `agent_loop` calls `rebuild_system_prompt_for_tool_loop` after every tool batch, which re-reads the workspace files and replaces the system message outright — corrected in #1310. A whole batch still executes between two rebuilds, and the first batch of a run runs against the context build's snapshot, so the argument below survives with a shorter window.) A mutual-exclusion primitive can only bracket a critical section, and there is no critical section here — the stale snapshot is already in the model's context before any lock is taken. So a `mode: "write"` overwrites whatever was appended inside that window, and #1292/#1294 structurally could not reach that (#1305). **A second agent is not required**: a run that appends a few memories and then tidies up with an explicit `mode: "write"` before finishing erases its own earlier appends, single-agent, no concurrency at all.

#1305 narrowed the exposure rather than closing it, at the only place it could without a protocol change: an omitted `mode` on `memories` now resolves to `"append"` instead of `"write"` (`WorkspaceFile::default_write_mode`), so the branch an LLM takes when it does not think to ask for one is no longer the destructive one. **The residual survived an explicit `mode: "write"` until #1310, which closed it** — a compare-and-swap that refuses a replacement when the file on disk is not what the agent was shown in full, plus a `workspace_read` tool for the case that genuinely needs the whole file. Two things this paragraph used to say are wrong and are corrected rather than deleted, because both were load-bearing here: the agent was never without a way to re-read (the workspace sits inside the sandbox root, so `fs_read` reaches it by path — the real gap was that no tool read a workspace file *by name*, nothing handed the model the path, and `fs_read` is not guaranteed enabled); and a rejection **carrying the file contents** is *not* the shape the fix should take. Sizing that payload to be safe means sending the same window the injection already sent, which an agent still cannot correctly rewrite a file from; sizing it to be sufficient means the whole file, unbounded, on every refusal, persisted in session history. #1310 sends neither and names the recovery instead. What remains for the queue is only the weaker problem above — both instances write, neither read the other, and the resulting file is a coherent-looking merge of two conversations that never met.
...
| **Transcript** | Only the invoking parent, and that *is* an access boundary since #1298: `read_subagent_session` and `read_session` both call `alms_core::subagent_session_access`, which authorizes on the parent embedded in the `context_id` (#1181/#1185). Until then `read_session` authorized on `session.agent_id` and so admitted the invoked agent to the same bytes -- discovery, not authorization, was what kept that latent (`list_my_sessions` filters `subagent` out) | Readable by both peers (`read_messages`) |
...
- **`ConversationEndReason` enum** (`crates/alms-tools/src/message_sender.rs`): `Ignored` (agent called `ignore_message`), `DepthExceeded` (MAX_DM_DEPTH hit), `UserCancelled` (operator cancelled the run mid-flight), or `Errored { message, interrupted }` (the run failed — LLM error, tool error, posture trip, etc.; the sanitised/truncated error message rides along for peer-side rendering). `interrupted` distinguishes a run that **died** mid-turn from one that completed with an unusable result, and `is_interrupted()` folds that together with `UserCancelled` into the predicate `run_trigger_loop` uses to decide whether the end costs an LLM turn (#1258). It is not on the wire — `Display` is `"errored"` either way. Included in the `dm_ended` marker metadata and the `ConversationEnded` `RunTrigger`.

- **Notification sessions**: Context ID pattern `notifications:{agent_name}`, one per agent. When the agent initiated the DM from a user-facing session, the `MessageBus` routes the `ConversationEnded` trigger directly to that source session (via `source_session_id`). When `source_session_id` is `None` (the agent was a pure DM recipient), the notification run stays on the `notifications:` session — the gateway does NOT reroute it. A lightweight `dm_conversation_ended` SSE event + marker message is sent to the web-chat separately by `notify_dm_ended_to_webchat`. These sessions do NOT start with `dm:`, so *context-id-driven* DM detection naturally skips DM-specific behavior (no DM addendum, no perspective mapping). Notification sessions are excluded from user-facing context (e.g. the `user.md` workspace file).

  **The `send_message` fold is the deliberate exception (#1299).** A post-end turn must not be able to message the peer whose conversation just ended: `MAX_DM_DEPTH` bounds one conversation, `end_conversation` resets the counter, and the notification run hands the agent the transcript — so an unfolded `send_message` re-opens the same conversation at depth 1, indefinitely. The fold is therefore keyed on the peer carried by the `ConversationEnded` trigger, **not** on the context id, and it is applied to every run the trigger produces (`plan_triggered_runs`). Context-id detection could not do this job: the run lands on a `notifications:`, web-chat, or — when the end resolves an open job episode (#1198 / #1205) — `job_*` session, and none of those name the peer. The job arm matters most: it is the one that survives the #1258 interrupted-end suppression, so it can be the only run an end produces, and it re-opens with nobody watching.

  The fold removes exactly one recipient for exactly one turn. `send_message` to any third party still delivers, and nothing prevents the pair re-opening later by ordinary means — see § 14.3 for why no pair-level re-open counter exists.
...
4. **Post-end `send_message` fold** (#1299, implemented): mitigation 2 bounds a *conversation*, not a *pair* — `end_conversation` clears the depth counter, so the next `send_message` between the same two agents starts again at depth 1. The gap that made that unbounded was the post-end turn itself: the agent is handed the transcript of the conversation that just ended and could message the peer straight back. `send_message` toward that peer is now folded for that turn (see § 9.5).

**Rejected: a pair-level re-open counter** (decision recorded for #1299).

Counting *conversations* between a pair, rather than messages within one, was considered and rejected:

- **The loop the fold closes is the automatic one.** cap → end → re-open → cap needs no new intent, only a turn in which the peer is addressable. Every re-entry that survives the fold requires an *external* trigger in a later turn — a user message, a cron tick, or a third agent's decision — i.e. a fresh reason to talk.
- **A counter's false-positive population is the legitimate population.** It fires on exactly those external triggers. Its likeliest first victim is a recurring job whose entire purpose is periodic two-agent coordination. Any threshold low enough to stop a runaway pair is low enough to break that job, and there is no state that distinguishes "the same conversation again" from "the next scheduled one".
- **Its failure mode is silent.** An exhausted counter makes `send_message` stop working between two agents with no operator signal and no reset story — the shape of the "DM randomly stops" class of bug (#1195).
- **`ActivityStamp` / `DEPTH_EXPIRY_SECS` is the wrong instrument.** That machinery encodes staleness, not re-entry; building a re-open bound on it would make the stall above time-dependent and unreproducible.

If unbounded pairs are ever observed *despite* the fold, the evidence to collect first is which turn re-opened and why — a counter would hide it.
