...
What no lock covers is a caller that read the file, thought, and then replaced it. The agent's "read" of its memories is the **context build**, which runs *once per run*: the system prompt is assembled before `agent_loop` and carried through every tool iteration, so the snapshot a `workspace_write` replaces is as old as the run, not one turn old. A mutual-exclusion primitive can only bracket a critical section, and there is no critical section here — the stale snapshot is already in the model's context before any lock is taken. So a `mode: "write"` overwrites whatever was appended inside that window, and #1292/#1294 structurally could not reach that (#1305). **A second agent is not required**: a run that appends a few memories and then tidies up with an explicit `mode: "write"` before finishing erases its own earlier appends, single-agent, no concurrency at all.

#1305 narrowed the exposure rather than closing it, at the only place it could without a protocol change: an omitted `mode` on `memories` now resolves to `"append"` instead of `"write"` (`WorkspaceFile::default_write_mode`), so the branch an LLM takes when it does not think to ask for one is no longer the destructive one. **The residual survives an explicit `mode: "write"`.** It is not, however, unfixable — the shape of the fix is known and is recorded here so nobody cites this paragraph as a reason to stop looking. A *bare* rejection would be unusable, because the agent has no way to re-read its workspace: there is no `workspace_read` tool, only the system-prompt injection it has already consumed. A rejection **carrying the file contents** would be usable, and is the shape any future fix should take — a failed tool call already reaches the model as an `Error: …` tool result it can act on, and `WorkspaceWriteTool` is constructed per run one hop from the context build, so a compare-and-swap needs no new tool and no schema change. That is the stronger form of what the queue is still needed for; the weaker one is the problem above — both instances write, neither read the other, and the resulting file is a coherent-looking merge of two conversations that never met.
