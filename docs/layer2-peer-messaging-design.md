...
The memory bullet used to read *corruption*, and meant it literally: `AgentWorkspace::append_file` was an unlocked read-modify-write, so one of two overlapping appends was silently dropped. That is fixed (#1280, #1294): every writer — `append_file`, `workspace_write` with `mode: "write"`, and `PUT /agents/{id}/workspace/{file}` — now runs under the file's exclusive advisory lock, appends land through an append-mode handle, and replacements land through a rename. So no *write* can land inside another's, and no reader ever observes a partial file, without any caller-side serialisation. What that does not cover is a caller that read the file, thought, and then replaced it: a `mode: "write"` still overwrites any append that landed in between, because the read-modify-write spans a whole run and no file lock can bracket that. That is the stronger form of what the queue is still needed for; the weaker one is the problem above — both instances write, neither read the other, and the resulting file is a coherent-looking merge of two conversations that never met.
