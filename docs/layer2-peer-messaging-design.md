...
The peer receives a one-shot notification run — **unless the end was *interrupted***, in which case it gets no run at all and the persisted marker plus its live banner are the delivery (#1258; see `dm-run-lifecycle.md` hazard 8 for the authoritative description of which reasons are interrupted and why the cut is not the transcript). Everything in the rest of this section describes the run that a *completed* end still gets. The raw `RunTrigger.input` from `end_conversation()` is a simple marker, but `run_trigger_loop` in `runs.rs` enriches it via `format_dm_ended_notification()`, which produces a richer message including the reason and a `read_messages` hint. For example, when the reason is `Ignored`:
