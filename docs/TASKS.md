...
- A tool that lets a running agent spawn a subagent with its own system prompt and tool set.
- Returns the subagent's final response as a string tool result.
- Requires the Coordinator to be wired into execute_run (see task #29).
- **Owners:** Atlas
...
- Current state: alms-coordinator has the full scaffolding (Coordinator, SubagentHandle,
  AgentMessage bus, spawn_subagent, cancel_subagent) but execute_task() is a placeholder
  that simulates work with tokio::time::sleep and fake progress events.
- Plan:
  a) Replace execute_task() placeholder with a real AgentRuntime::run() call.
     The subagent gets its own AgentId, session context, tool registry, and system prompt
     derived from SubagentRequest.agent_type.
  b) Wire Coordinator into AppState — create one Coordinator per gateway instance,
     share it via Arc<Coordinator>. execute_run() receives a Coordinator handle.
  c) Add invoke_agent tool (task #28) that calls Coordinator::spawn_subagent() and
     awaits TaskResult, returning the subagent response as a tool result string.
  d) Expose coordinator state via HTTP: GET /tasks (list active), GET /tasks/{id} (status).
  e) Forward subagent SSE events (progress, tool_start/end) into the parent run's event stream
     so the UI can show subagent activity inline.
- Dependency: task #28 (invoke_agent tool) depends on this.
- **Owners:** Atlas
