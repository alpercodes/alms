  ...
  "tool_calls": [
    {
      ...
      "tool_name": "shell",
      "tool_id": "call_abc123",
      "tool_invocation_id": "9f2b1c44-0e6a-4b1d-9a77-1c2d3e4f5061",
      "params": "{\"command\":\"ls\"}",
...
| `tool_id` | string | optional | Provider-assigned tool call ID that correlates a call to its result. This is the *LLM provider's* id and appears in no SSE event — see `tool_invocation_id`. |
| `tool_invocation_id` | string (UUID) | optional | **ALMS's own invocation correlator** (#5) — the id carried by the `tool_start` / `tool_end` / `subagent_started` / `subagent_completed` SSE events and written into `session_messages` metadata under the same name. A client reconstructing a tool row from these records should key it on this field so the row matches the live event stream, falling back to `tool_id` when absent. Absent on records written before this field existed. |
...
- **Correlating a reconstructed row with the live stream.** `tool_id` and `tool_invocation_id` are different identifiers with different issuers. `tool_id` comes from the LLM provider and pairs a call record with its result record; `tool_invocation_id` is generated by ALMS and is the only one the event stream, the session-message metadata and the UI's identity lookups ever see. Before #5 only `tool_id` was stored, so a row rebuilt from this endpoint could not be matched against an incoming `tool_end` at all. Records written before that change carry no `tool_invocation_id`; clients should fall back to `tool_id`, which is the pre-#5 behaviour.
...
- `tool_invocation_id` (optional string, UUID) is ALMS's own invocation correlator — the id the `tool_start` / `tool_end` SSE events carry (#5). `tool_id` is the LLM provider's id and appears in no event. See the correlation note in section 4.6. Absent on records written before the field existed.
...
> **Note:** When a run is system-triggered (peer-to-peer DMs via `send_message`, notification runs, subagent completions, and scheduled jobs), Guarded posture is automatically overridden to Autonomous for that run via the `is_system_triggered` flag, since there is no human in the loop to approve tool calls; a Guarded agent run as a background subagent is overridden the same way. Without this override the run would block on its first gated tool call until the `max_run_duration_secs` backstop. User-initiated runs (via the HTTP API) are never affected — Guarded posture is preserved as configured. Guarded therefore governs the runs a human starts and is not a boundary between agents: an agent that can send a Guarded agent a DM can have it run tools without approval. See [`security-model.md` § 8.1](security-model.md#guarded-is-not-an-agent-boundary); the behaviour is under discussion in [#177](https://github.com/alpercodes/alms/issues/177).
...
Within `session`, `max_context_tokens` must be at least `[context].max_input_tokens` — session storage has to hold one full context window. The rule is evaluated on the **would-be post-patch** values, and only for bodies that carry a `session` block *and* name one of its two halves (`session.max_context_tokens` or `context.max_input_tokens`); a body naming neither cannot move the two relative to each other, so it is not judged against them. The `session` section is committed **all or nothing**: when the rule fails, every session field in the body stays at its pre-PATCH value and nothing is persisted (#1275). Before #1275 the named fields were committed first and `max_context_tokens` was then "reverted" to `[context].max_input_tokens`, so a rejected PATCH could leave — or introduce — a value the operator never sent, including on a body that never named the field.
