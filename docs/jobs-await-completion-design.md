...
**Episode persistence (phase 2):** the deadline means closing #1159 first is no longer a *hard* prerequisite — a persisted episode that survives a restart with dead triggers now self-heals at the deadline instead of hanging forever. It is still strongly preferable to land #1159 first (a 4-hour silent stall on every restart-interrupted episode is poor behavior, just no longer unbounded). In phase 1 episodes remain in-memory: a restart drops the episode; recurring jobs self-heal at the next tick, and the stale past-due `next_run_at` is re-fired by `bootstrap_scheduler` (`bootstrap_fire_at` in `crates/alms-gateway/src/server/mod.rs`). That re-fire was an *accidental* retry when this doc was written; #1230 made it **deliberate and universal** — a persisted `next_run_at` now wins for every schedule type, so any past-due tick is caught up at boot rather than skipped. #1235 then staggered that catch-up cohort so a restart after long downtime spreads the missed firings instead of running them all at once; see `BOOT_CATCH_UP_SPACING_SECS`. `Once` jobs are lost like any `Queued` run today.
...
4. **Pending subagents:** for each `PendingWork::Subagent`, fire the existing subagent cancel path (`cancel_subagent` / handle token). The resulting `SubagentCompletion(Cancelled)` finds no episode (already removed). Its notification run is suppressed at the source because `enqueue_triggered_run` checks `AppState.operator_cancelled_jobs`, populated only by `DELETE /jobs`. This explicit intent is a synchronous race fence for producers holding older lifecycle snapshots; completed/failed jobs are never inserted, so deadline-detached late results remain deliverable.
...
- **The `notifications:{agent}` DM-ended run:** for job-originated DMs this path is superseded by D3 + the tracker override (continuation lands on the job session). For all non-job DMs, routing is byte-identical to today. If a `ConversationEnded` arrives after its episode closed, an explicitly operator-cancelled job suppresses the orphan notification run, while a completed deadline-closed episode still delivers the detached result through an orphan run.
