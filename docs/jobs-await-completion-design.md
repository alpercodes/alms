...
4. **Pending subagents:** for each `PendingWork::Subagent`, fire the existing subagent cancel path (`cancel_subagent` / handle token). The resulting `SubagentCompletion(Cancelled)` finds no episode (already removed) and degrades to today's behavior — a notification run on the (job) parent session, which is harmless post-cancel and is itself cancelled by (2) if still queued.
