...
**Size management:**
- `memories.md` is truncated at 4000 characters before injection (hard limit in `build_system_prompt_prefix`). ⚠️ The truncation is **head-anchored** — `truncate_to_char_boundary` returns `&s[..end]`, so the injected window is the *oldest* 4000 chars plus a `[memories truncated, N chars total]` marker. Since #1305 made `memories` append by default, the file grows at the tail while the window stays anchored at the head, so **past the cap, newly appended memories stop being injected at all**, and the "visible next context build, repairable" property claimed for the append default below stops holding with it. Head-anchoring is the wrong end to cut for an append-default file; tracked separately, not resolved by #1305
