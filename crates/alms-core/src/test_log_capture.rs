...
//! binaries are separate processes, and each needs its own global
//! capture subscriber. Keep them in step.
//!
...
//! in-memory buffer. That is what the log-asserting tests here used to
//! do, and it is subtly, intermittently wrong.
//!
...
fn install_global_subscriber() {
    ...
    INSTALLED.get_or_init(|| {
        ...
        tracing::subscriber::set_global_default(subscriber)
            .expect("alms-core's test binary must not install any other global tracing subscriber");
    });
}
...
mod tests {
    ...
    /// flake this module exists to kill. (The gateway copy of this
    /// module carries the same two guards plus thread-locality and level
    /// coverage; this binary only needs the load-bearing ones.)
    ///
