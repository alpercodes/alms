...
Context: `+t.context_id+_}
    ...
    `}function Pn(e){let t=new Set,n=[];for(let r of e){if(r.session_type!==`dm`||!Array.isArray(r.participants)||r.participants.length<2)continue;let e=r.context_id||r.id;t.has(e)||(t.add(e),n.push(r))}return n}function Fn(){let e=v.value,t=I.value,n=B.value,r=z.value,i=oe.value,a=Pe.value?Pe.value.name:null,o=ht(e.filter(e=>e.session_type!==`dm`&&e.session_type!==`notification`&&e.session_type!==`job`&&e.session_type!==`subagent`&&e.session_type!==`episodic`)),s=ht(bt(t)),c=ht(t.filter(e=>e.session_type!==`dm`&&e.session_type!==`notification`&&e.session_type!==`job`)),l=_t(Pn(t),a),u=_t(t.filter(e=>e.session_type===`notification`),a),f=yt(t),p=x.value;return d`
...
`)}</pre>
    ...
    `}function yo(){let e=Pe.value,{phase:t,detail:n}=De.value,r=be(t,n),o=a(!1),s=a(!1),u=c(null),f=l(()=>{o.value=!o.value},[]),p=l(()=>{o.value=!1,s.value=!0},[]);return i(()=>{if(!o.value)return;let e=e=>{u.current&&!u.current.contains(e.target)&&(o.value=!1)};return document.addEventListener(`click`,e,!0),()=>document.removeEventListener(`click`,e,!0)},[o.value]),e?d`
