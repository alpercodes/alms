...
`)}</pre>
    ...
    `:d`<span class="model-display model-display--muted">unknown</span>`}function Gi(e){return e===`Enter`||e===` `}function Ki(e){return!e||typeof e.key!=`string`||e.defaultPrevented?!1:Gi(e.key)}function qi(e){return!(!e||e.defaultPrevented)}function Ji(e,t){let n=!!e,r=!!t;return n===r?{}:{debug_mode:r}}var Yi=`Emits a snapshot of the context window sent to the LLM on every turn, rendered below the chat. Applies from the next run.`;function Xi(e){return`Each ${e} applies only when the agent is on that provider; the others are ignored.`}var Zi=`Optional cheaper model for compaction summaries and episodic memory. Set provider and model together, or neither.`,Qi=[`minimal`,`low`,`medium`,`high`];function $i(e){return e==null?`inherit`:e===0?`disable`:`custom`}function ea(e){return e==null||e===``?`inherit`:`custom`}function ta({label:e,agentValue:t,mode:n,draft:r}){return d`
    ...
    `}function na({label:e,mode:t,value:n}){return d`
        <div class="settings-row">
            <label class="settings-label">${e}</label>
            <div class="agent-tristate">
                <select class="settings-select agent-tristate-mode"
    ...
    `}async function ra(){try{let e=await ji();Pe(e.agents||e||[])}catch(e){console.error(`[agents] fetch failed:`,e)}}function ia({agent:e,onClose:t}){let n=a(e.description||``),r=a(e.model||``),i=a(e.posture||``),o=a(e.provider||``),s=a(`keep`),c=a(``),l=a($i(e.thinking_budget_tokens)),u=a(e.thinking_budget_tokens&&e.thinking_budget_tokens>0?String(e.thinking_budget_tokens):``),f=a(ea(e.reasoning_effort)),p=a(e.reasoning_effort||``),m=a($i(e.gemini_thinking_budget)),h=a(e.gemini_thinking_budget&&e.gemini_thinking_budget>0?String(e.gemini_thinking_budget):``),g=a(e.summary_provider||``),_=a(e.summary_model||``),v=a(!!e.debug_mode),y=a(!1),b=a(``),x=H.value.model||``,S=H.value.provider||``,C=H.value.llm_providers||[],w=()=>{let t={};n.value!==(e.description||``)&&(t.description=n.value),(r.value||``)!==(e.model||``)&&(t.model=r.value||``),(i.value||``)!==(e.posture||``)&&(t.posture=i.value||``),(o.value||``)!==(e.provider||``)&&(t.provider=o.value||``),s.value===`set`&&c.value.trim()?t.telegram_token=c.value.trim():s.value===`remove`&&(t.telegram_token=``);let a=e.thinking_budget_tokens;if(l.value===`inherit`)a!=null&&(t.clear_thinking_budget_tokens=!0);else if(l.value===`disable`)a!==0&&(t.thinking_budget_tokens=0);else if(l.value===`custom`){let e=parseInt(u.value,10);!isNaN(e)&&e>=0&&e!==a&&(t.thinking_budget_tokens=e)}let d=e.reasoning_effort||null;f.value===`inherit`?d!=null&&(t.clear_reasoning_effort=!0):f.value===`custom`&&p.value&&p.value!==d&&(t.reasoning_effort=p.value);let y=e.gemini_thinking_budget;if(m.value===`inherit`)y!=null&&(t.clear_gemini_thinking_budget=!0);else if(m.value===`disable`)y!==0&&(t.gemini_thinking_budget=0);else if(m.value===`custom`){let e=parseInt(h.value,10);!isNaN(e)&&e>=0&&e!==y&&(t.gemini_thinking_budget=e)}let b=e.summary_provider||``,x=(g.value||``).trim();x!==b&&(x===``?t.clear_summary_provider=!0:t.summary_provider=x);let S=e.summary_model||``,C=(_.value||``).trim();return C!==S&&(C===``?t.clear_summary_model=!0:t.summary_model=C),Object.assign(t,Ji(e.debug_mode,v.value)),t},T=async()=>{y.value=!0,b.value=``;try{let n=w();if(Object.keys(n).length===0){t();return}await Ni(e.id,n),await ra(),t()}catch(e){b.value=e.error?.message||e.message||`Save failed`}finally{y.value=!1}},E=e=>{e.target===e.currentTarget&&t()},D=!!e.has_telegram;return d`
        <div class="settings-overlay open" onClick=${E}>
            <div class="settings-modal agent-edit-modal">
                ...
                <span class="settings-hint">${Xi(`row`)}</span>
                ...
                <span class="settings-hint">
                    ${Zi} Leave both empty to use the server default.
                </span>
    ...
    `}function aa({agent:e,isActive:t,onEdit:n}){let r=a(``),i=a(!1),o=a(null),s=H.value.model||``,c=H.value.provider||``;return d`
        <div class="agent-card ${t?`active`:``}"
            ...
            <div class="agent-card-actions">
                ...
                ${!e.is_default&&d`
                    <button class="agent-card-btn" onClick=${async t=>{t&&t.stopPropagation();try{await Fi(e.id),await ra()}catch(e){r.value=e.error?.message||e.message||`Failed`}}}>Set Default</button>
                `}
                ${i.value?d`
                        <button class="agent-card-btn" style="color:var(--error); font-weight:600;" onClick=${async t=>{t&&t.stopPropagation(),o.value&&=(clearTimeout(o.value),null),i.value=!1;try{if(await Pi(e.id),await ra(),e.id===z.value){let e=B.value.find(e=>e.is_default)||B.value[0]||null;e?Nt(e.id):(z.value=null,oe.value=null)}}catch(e){r.value=e.error?.message||e.message||`Delete failed`}}}>Confirm?</button>
    ...
    `}function oa(){let e=a(``),t=a(``),n=a(!1),r=a(null);i(()=>{X.value===`agents`&&ra()},[X.value]);let o=nt(e.value),s=(e.value||``).trim(),c=s!==``&&o!==s,l=async()=>{let r=nt(e.value);if(!r){(e.value||``).trim()===``?t.value=`Agent name is required`:t.value=`Agent name must contain at least one letter or digit`;return}let i=tt(r);if(i){t.value=i.message;return}t.value=``,n.value=!0;try{let t=await Mi({name:r});t.id||console.warn(`[agents] POST /agents returned no id for agent:`,r,t),e.value=``,await ra()}catch(e){t.value=e.error?.message||e.message||`Failed to create agent`}finally{n.value=!1}};return d`
    ...
    `}var sa=e=>O(`/agents/${e}/workspace`),ca=(e,t,n)=>y(`/agents/${e}/workspace/${t}`,{content:n}),la=e=>_(`/agents/${e}/workspace/open`,{}),ua=[`personality`,`goals`,`memories`,`user`];async function da(){if(!z.value){U.value=null;return}try{let e=await sa(z.value);U.value=e.files||e}catch(e){e.status===404||e.error?.code===`NOT_FOUND`?U.value=`unavailable`:U.value=`error`}}function fa({agentId:e,doOpen:t}){let n=a(!1),r=a(null),i=async()=>{if(!(n.value||!e)){n.value=!0,r.value=null;try{await t(e),r.value={kind:`ok`,text:`Opened`},setTimeout(()=>{r.value?.kind===`ok`&&(r.value=null)},2e3)}catch(e){let t=e?.error?.code,n=e?.error?.message||e?.message||`Failed to open workspace`,i=n;t===`NOT_CONFIGURED`?i=`Workspace dir not configured`:t===`WORKSPACE_PATH_MISSING`?i=`Workspace path is missing on disk`:t===`LAUNCHER_FAILED`&&(i=`Failed to launch file explorer`),r.value={kind:`err`,text:i,full:n}}finally{n.value=!1}}};return d`
    ...
    `}function pa({agentId:e,filename:t,content:n}){let r=a(n||``),o=a(``),s=a(!1);return i(()=>{r.value=n||``},[n]),d`
        <div class="ws-file">
            ...
            <div style="display:flex; align-items:center; gap:var(--space-2);">
                <button class="ws-save" onClick=${async()=>{if(!s.value){s.value=!0,o.value=``;try{await ca(e,t,r.value),o.value=`Saved`,setTimeout(()=>{o.value=``},2e3),await da()}catch(e){o.value=`Error: `+(e.error?.message||e.message||`save failed`)}finally{s.value=!1}}}} disabled=${s.value}>
                    ${s.value?`Saving...`:`Save`}
                </button>
                ${o.value&&d`
    ...
    `}function ma(){return i(()=>{X.value===`workspace`&&da()},[X.value,z.value]),z.value?U.value===null?d`<div class="loading-state">Loading...</div>`:U.value===`unavailable`?d`<div class="ws-notice">Workspace not configured for this agent</div>`:U.value===`error`?d`<div class="ws-notice" style="color:var(--error);">Failed to load workspace</div>`:d`
        <div>
            <${fa}
                agentId=${z.value}
                doOpen=${la} />
    ...
    `:d`<div class="ws-notice">No agent selected</div>`}var ha=R.jobs;function ga(){return R.getJobMutationGeneration()}function _a(e,t){R.replaceJobs(e,t)}function va(e){R.createOptimisticJob(e)}function ya(e,t){R.confirmOptimisticJobCreate(e,t)}function ba(e){R.rollbackOptimisticJobCreate(e)}function xa(e){R.cancelOptimisticJob(e)}function Sa(e,t){R.confirmOptimisticJobCancel(e,t)}function Ca(e){R.rollbackOptimisticJobCancel(e)}var wa=()=>O(`/jobs`),Ta=e=>_(`/jobs`,e),Ea=e=>f(`/jobs/${e}`),Da=0,Oa=[{label:`1m`,cron:`* * * * *`,desc:`Every minute`},{label:`5m`,cron:`*/5 * * * *`,desc:`Every 5 minutes`},{label:`15m`,cron:`*/15 * * * *`,desc:`Every 15 minutes`},{label:`30m`,cron:`*/30 * * * *`,desc:`Every 30 minutes`},{label:`1h`,cron:`0 * * * *`,desc:`Every hour`},{label:`6h`,cron:`0 */6 * * *`,desc:`Every 6 hours`},{label:`12h`,cron:`0 */12 * * *`,desc:`Every 12 hours`},{label:`1d`,cron:`0 0 * * *`,desc:`Daily at midnight`}];function ka(e){return e===`pending`||e===`active`}function Aa(e){let t=e.status||`active`;return e.terminal_reason?`${t} (${e.terminal_reason.replaceAll(`_`,` `)})`:t}function ja(e){return e.retry_count?`${e.retry_count} dispatch ${e.retry_count===1?`retry`:`retries`}`:``}function Ma(e){if(!e)return``;let t=Oa.find(t=>t.cron===e.trim());return t?t.desc:e.trim().split(/\s+/).length===5?e:`Invalid cron (need 5 fields)`}function Na(e){let t=e=>String(e).padStart(2,`0`);return`${e.getFullYear()}-${t(e.getMonth()+1)}-${t(e.getDate())}T${t(e.getHours())}:${t(e.getMinutes())}`}function Pa(){let e=new Date(Date.now()+5*6e4);return e.setSeconds(0,0),Na(e)}function Fa(){let e=new Date;return e.setSeconds(0,0),Na(e)}async function Ia(){let e=ga();try{let t=await wa();_a(t.jobs||t||[],e)}catch(e){console.error(`[jobs] fetch failed:`,e)}}function La(){let e=a(`recurring`),t=a(``),n=a(Pa()),r=a(``),o=a(z.value||``),s=a(``),c=a(``),l=a(!1),u=a(!1);i(()=>{X.value===`jobs`&&Ia()},[X.value]),i(()=>{o.value=z.value||``},[z.value]);let f=Ma(t.value),p=e.value===`once`?!!n.value:!!t.value.trim(),m=!!o.value&&p&&!!r.value.trim(),h=async()=>{if(!m)return;s.value=``,c.value=``,l.value=!0;let i=null;try{let a;if(e.value===`once`){let e=new Date(n.value);if(isNaN(e.getTime())){s.value=`Invalid date/time. Please select a valid date.`,l.value=!1;return}a={type:`once`,run_at:e.toISOString()}}else a={type:`recurring`,cron:t.value.trim()};let d={agent_id:o.value,schedule:a,prompt:r.value.trim()};i=`optimistic-job-`+ ++Da,va({id:i,...d,status:`pending`,next_run_at:null,last_run_at:null});let f=await Ta(d);ya(i,f),i=null,t.value=``,r.value=``,u.value=!1,n.value=Pa(),c.value=e.value===`once`?`Job scheduled (one-time).`:`Recurring job created.`,setTimeout(()=>{c.value=``},4e3)}catch(e){i&&ba(i);let t=e.error?.message||e.message||``;s.value=t||`Failed to create job. Check that all fields are filled and the schedule is valid.`}finally{l.value=!1}},g=async e=>{s.value=``,xa(e);try{Sa(e,await Ea(e))}catch(t){Ca(e),t.status===409&&await Ia(),s.value=t.error?.message||t.message||`Failed to cancel job`}};return d`
        <div>
            <div class="jobs-form">
            ...
            ${ha.value.length===0?d`<div class="jobs-empty">No scheduled jobs</div>`:ha.value.map(e=>d`
                    <div class="job-item">
                        <div class="job-prompt">${e.prompt||e.task||`(no prompt)`}</div>
                        <div class="job-meta">
                            <span>${Ma(e.schedule?.cron)||(e.schedule?.type===`once`?`Once at `+Vn(e.schedule.run_at):JSON.stringify(e.schedule))}</span>
                            ${e.next_run_at&&d`<span> | next: ${Vn(e.next_run_at)}</span>`}
                            ${e.last_run_at&&d`<span> | last run: ${Vn(e.last_run_at)}</span>`}
                            ${ja(e)&&d`<span> | ${ja(e)}</span>`}
                            ${e.last_error&&d`<span class="job-last-error"> | last error: ${e.last_error}</span>`}
                        </div>
                        <span class="job-status-${e.status||`active`}">${Aa(e)}</span>
                        ${ka(e.status||`active`)&&!e.optimistic&&d`
                            <button class="job-cancel" onClick=${()=>g(e.id)}>Cancel</button>
                        `}
                    </div>
                `)}
        </div>
    `}var Ra=(e,t=50)=>O(`/audit?session_id=${e}&limit=${t}`),za=50;async function Ba(e){if(!S.value){W.value=null;return}try{let t=await Ra(S.value,e);W.value=t.events||t||[]}catch{W.value=[]}}function Va(){let e=a(za),t=a(!1);i(()=>{X.value===`audit`&&(e.value=za,Ba(za))},[X.value,S.value]);let n=async()=>{t.value=!0;try{let t=e.value+za;e.value=t,await Ba(t)}catch(e){console.error(`[AuditTab] loadMore failed:`,e)}finally{t.value=!1}};if(!S.value)return d`<div class="empty-state">No session selected</div>`;if(W.value===null)return d`<div class="loading-state">Loading...</div>`;if(W.value.length===0)return d`<div class="empty-state">No audit events</div>`;let r=W.value.length>=e.value;return d`
        <div>
            ${W.value.map((e,t)=>d`
    ...
    `}var Ha=50,Ua={completed:`✓`,failed:`✗`,cancelled:`⊘`,running:`⋯`},Wa={user:`user`,scheduled:`scheduled`,subagent:`subagent`,dm:`dm`,notification:`notif`,telegram:`telegram`},Ga={chat:`chat`,dm:`dm`,subagent:`sub`,job:`job`,notification:`notif`,telegram:`tg`};function Ka(e){if(e==null)return`--`;if(e<1e3)return e+`ms`;if(e<6e4)return(e/1e3).toFixed(1)+`s`;let t=Math.floor(e/6e4),n=Math.round(e%6e4/1e3);return t+`m`+(n>0?n+`s`:``)}function qa(e){return e==null?`--`:e>=1e4?(e/1e3).toFixed(0)+`k`:e>=1e3?(e/1e3).toFixed(1)+`k`:String(e)}function Ja(e){if(!e)return``;let t=Date.now()-new Date(e).getTime();if(t<0)return`just now`;let n=Math.floor(t/1e3);if(n<60)return n+`s ago`;let r=Math.floor(n/60);if(r<60)return r+`m ago`;let i=Math.floor(r/60);return i<24?i+`h ago`:Math.floor(i/24)+`d ago`}function Ya(){let e=a([]),t=a(!1),n=a(``),r=async()=>{if(!z.value){e.value=[];return}t.value=!0,n.value=``;try{let t=await Qe(z.value,Ha);e.value=t.runs||[]}catch(t){console.error(`[RunsTab] fetch failed:`,t),n.value=t.error?.message||t.message||`Failed to load runs`,e.value=[]}finally{t.value=!1}};return i(()=>{X.value===`runs`&&r()},[X.value,z.value,Re.value]),z.value?t.value&&e.value.length===0?d`<div class="loading-state">Loading runs...</div>`:n.value?d`
            <div>
                ...
                ${e.value.map(e=>d`
                    <div class="runs-tab-row runs-tab-row--${e.status||`unknown`}"
                        ...
                        <div class="runs-tab-row-top">
                            <span class="runs-tab-status">${Ua[e.status]||`·`}</span>
                            <span class="runs-tab-trigger runs-tab-trigger--${e.trigger||`user`}">
                                ${Wa[e.trigger]||e.trigger||`user`}
                            </span>
                            <span class="runs-tab-session-type">
                                ${Ga[e.session_type]||e.session_type||``}
                            </span>
                            <span class="runs-tab-time">${Ja(e.ts)}</span>
                        </div>
                        <div class="runs-tab-row-bottom">
                            <span class="runs-tab-duration">${Ka(e.duration_ms)}</span>
                            <span class="runs-tab-tools">${e.tool_call_count==null?``:e.tool_call_count+` tools`}</span>
                            <span class="runs-tab-tokens">
                                ${e.usage?qa(e.usage.prompt_tokens)+` in / `+qa(e.usage.completion_tokens)+` out`+(typeof e.usage.reasoning_tokens==`number`&&e.usage.reasoning_tokens>0?` (+`+qa(e.usage.reasoning_tokens)+` reasoning)`:``)+(typeof e.usage.cache_read_input_tokens==`number`&&e.usage.cache_read_input_tokens>0?` (`+qa(e.usage.cache_read_input_tokens)+` cached)`:``):``}
                            </span>
                        </div>
                    </div>
                `)}
    ...
    `:d`<div class="runs-tab-empty">No agent selected</div>`}function Xa(e,t=50,n=null){let r=`/agents/${e}/timeline?limit=${t}`;return n&&(r+=`&before=${encodeURIComponent(n)}`),O(r)}var Za=50,Qa={run_started:`▶`,run_completed:`✓`,run_failed:`✗`,run_cancelled:`⊘`,run_ended:`■`,tool_call:`⚙`,message_received:`●`,message_sent:`○`,marker:`⚑`},$a={run_started:`started`,run_completed:`completed`,run_failed:`failed`,run_cancelled:`cancelled`,run_ended:`ended`,tool_call:`tool`,message_received:`message`,message_sent:`sent`,marker:`marker`},eo={chat:`chat`,dm:`dm`,subagent:`sub`,job:`job`,notification:`notif`,telegram:`tg`,episodic:`epis`};function to(e){if(!e)return``;let t=Date.now()-new Date(e).getTime();if(t<0)return`just now`;let n=Math.floor(t/1e3);if(n<60)return n+`s ago`;let r=Math.floor(n/60);if(r<60)return r+`m ago`;let i=Math.floor(r/60);return i<24?i+`h ago`:Math.floor(i/24)+`d ago`}function no(e){return e?new Date(e).toLocaleTimeString([],{hour:`2-digit`,minute:`2-digit`}):``}function ro(e){if(!e)return``;let t=new Date(e),n=new Date,r=new Date;return r.setDate(r.getDate()-1),t.toDateString()===n.toDateString()?`Today`:t.toDateString()===r.toDateString()?`Yesterday`:t.toLocaleDateString([],{weekday:`short`,month:`short`,day:`numeric`})}function io(){let e=a([]),t=a(!1),n=a(!1),r=a(``),o=a(!1),s=a(null),c=async(i=!1)=>{if(!z.value){e.value=[];return}i?n.value=!0:t.value=!0,r.value=``;try{let t=i?s.value:null,n=await Xa(z.value,Za,t),r=n.events||[];i?e.value=[...e.value,...r]:e.value=r,o.value=n.pagination?.has_more||!1,s.value=n.pagination?.next_before||null}catch(t){console.error(`[TimelineTab] fetch failed:`,t),r.value=t.error?.message||t.message||`Failed to load timeline`,i||(e.value=[])}finally{t.value=!1,n.value=!1}};if(i(()=>{X.value===`timeline`&&c(!1)},[X.value,z.value]),!z.value)return d`<div class="tl-empty">No agent selected</div>`;if(t.value&&e.value.length===0)return d`<div class="loading-state">Loading timeline...</div>`;if(r.value)return d`
            <div>
                <div class="tl-error">${r.value}</div>
                <button class="tl-retry" onClick=${()=>c(!1)}>Retry</button>
            </div>
        `;if(e.value.length===0)return d`<div class="tl-empty">No activity yet</div>`;let l=new Set;{let t=``;for(let n of e.value){let e=ro(n.timestamp);e!==t&&(l.add(n),t=e)}}return d`
        <div class="tl-tab">
            <div class="tl-header">
            ...
            <div class="tl-list">
                ${e.value.map((e,t)=>{let n=ro(e.timestamp),r=l.has(e),i=e.event_type===`tool_call`,a=e.event_type===`run_started`||e.event_type===`run_completed`||e.event_type===`run_failed`||e.event_type===`run_cancelled`||e.event_type===`run_ended`,o=e.metadata?.tool_name,s=e.event_type+`-`+e.timestamp+`-`+(e.run_id||``)+`-`+t+(o?`-`+o:``);return d`
                        ${r&&d`
                        ...
                        <div class="tl-event tl-event--${e.event_type}${i?` tl-event--indent`:``}${a?` tl-event--run`:``}"
                            ...
                            <span class="tl-time">${no(e.timestamp)}</span>
                            ...
                            <span class="tl-session-badge tl-session-badge--${e.session_type||`chat`}">
                                ${eo[e.session_type]||e.session_type||`chat`}
                            </span>
                            <span class="tl-event-label">${$a[e.event_type]||e.event_type}</span>
                            <span class="tl-ago">${to(e.timestamp)}</span>
                        </div>
                        ${e.summary&&d`
    ...
    `}function ao({tab:e}){return e===`agents`?d`<${oa} />`:e===`workspace`?d`<${ma} />`:e===`runs`?d`<${Ya} />`:e===`jobs`?d`<${La} />`:e===`audit`?d`<${Va} />`:e===`timeline`?d`<${io} />`:null}function oo(){Y.value=null}function so(){return Y.value?d`
    ...
    `:null}var co=()=>O(`/auth/keys`),lo=(e,t)=>y(`/auth/keys`,{provider:e,key:t}),uo=e=>f(`/auth/keys/${e}`),fo=[`openai`,`anthropic`,`openrouter`,`gemini`];function Q({title:e,defaultOpen:t=!1,children:n}){let r=a(t);return d`
    ...
    `}function mo(){let e=a([]),t=a(null),n=a(``),r=a(!1),o=a(``),s=async()=>{try{let t=await co();e.value=t.keys||[]}catch(e){console.error(`[auth] list keys failed:`,e)}};i(()=>{s()},[]);let c=async e=>{if(n.value.trim()){r.value=!0,o.value=``;try{await lo(e,n.value.trim()),n.value=``,t.value=null,await s()}catch(e){o.value=e.error?.message||e.message||`Failed to save key`}finally{r.value=!1}}},l=async e=>{try{await uo(e),await s()}catch(e){o.value=e.error?.message||e.message||`Failed to remove key`}};return d`
        <div class="settings-row">
            <label class="settings-label">API Keys</label>
            ${fo.map(i=>{let a=e.value.find(e=>e.provider===i),o=a?.configured,s=a?.source||`none`,u=a?.key||``;return t.value===i?d`
        ...
        <div class="settings-overlay open" onClick=${ce}>
            <div class="settings-modal">
                ...
                <${Q} key="defaults" title="Default LLM (model / provider)" defaultOpen=${!0}>
                    ...
                    <${$} label="Default LLM provider"
                        ...
                        <select class="settings-select settings-input-sm"
                            ...
                            ${(j.llm_providers&&j.llm_providers.length>0?j.llm_providers:fo).map(e=>{let t=Vi(e);return d`<option value=${e} key=${e}>${t===`Custom`?e:t}</option>`})}
                        </select>
                    <//>
                ...
                <${Q} key="summary" title="Summary" defaultOpen=${!1}>
                    <span class="settings-hint settings-section-desc">
                        ${Zi} Agents can override both in the Agents panel.
                    </span>
                    <${$} label="Summary model">
                        <input class="settings-input settings-input-sm" type="text"
                ...
                <${Q} key="llm" title="LLM Providers" defaultOpen=${!1}>
                    <span class="settings-hint settings-section-desc">
                        Reasoning and caching defaults. ${Xi(`block`)}
                    </span>
    ...
    `}var go=`openrouter`;function _o(e){let t=e&&e.keys;return Array.isArray(t)?t.some(e=>e&&e.configured===!0&&fo.includes(e.provider)):!1}var vo=`https://openrouter.ai/keys`;function yo({onSaved:e,onSkip:t}){let n=a(``),r=a(``),o=a(!1),s=c(null);return i(()=>{s.current?.focus()},[]),d`
