...
impl MainAgent {
    ...
    async fn collect_results(&mut self, task_ids: &[TaskId]) -> AlmsResult<()> {
                ...
                if let Some(status) = self.coordinator.get_status(*task_id) {
                    match status {
                        TaskStatus::Completed | TaskStatus::Failed | TaskStatus::Cancelled => {
                            ...
                            self.task_results.insert(*task_id, TaskResult {
                                task_id: *task_id,
                                status,
        ...
        if completed < task_ids.len() {
            warn!("Timeout waiting for {} subagents", task_ids.len() - completed);
        }
    ...
    async fn synthesize_response(&self, original_input: &str, task_ids: &[TaskId]) -> AlmsResult<String> {
        let mut parts = vec![format!("Based on your request: \"{}\"", original_input)];
        
        for task_id in task_ids {
            if let Some(request) = self.pending_tasks.get(task_id) {
                parts.push(format!("\n**{} Subagent**:", format!("{:?}", request.agent_type)));
